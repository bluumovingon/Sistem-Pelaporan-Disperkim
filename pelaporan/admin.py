...
class CustomUserAdmin(UserAdmin):
    ...
    fieldsets = UserAdmin.fieldsets + (
        ('Informasi Tambahan', {'fields': ('role', 'jabatan', 'unit_kerja', 'status_aktif')}),
    )
...
class LaporanAdmin(admin.ModelAdmin):
    list_display = ['id', 'kegiatan', 'pptk', 'bulan_laporan', 'status', 'updated_at']
    list_filter = ['status', 'bulan_laporan']
    search_fields = ['kegiatan__judul_kegiatan', 'pptk__username', 'tahapan_pelaksanaan']
